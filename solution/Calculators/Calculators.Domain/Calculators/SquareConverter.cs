...
{
    [Calculator("Конвертеры", "Конвертер площади", "SquareConverter")]
    public class SquareConverter : Calculator
    {
        private const int coeff = 10000;

        [UIHint("Enum")]
        [Required(ErrorMessage = "Выберите значение")]
        [Display(Name = "Единицы площади")]
        ...
        public override CalculateResult Calculate()
        {
            ...
            var values = Enum.GetValues(typeof(global::Calculators.Domain.Entities.Enums.Square));
            double currentValueType = (int)Square;
            currentValueType /= coeff;
            foreach (Square item in values)
            {
